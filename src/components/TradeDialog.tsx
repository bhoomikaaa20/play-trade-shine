...
export default function TradeDialog({
  ...
  const submit = async () => {
    ...
    setLoading(true);
    const fn = isBuy ? "trade-buy" : "trade-sell";
    const { data, error } = await supabase.functions.invoke(fn, {
      body: { asset_id: asset.id, quantity },
    });
    setLoading(false);
    ...
    toast.success(`${isBuy ? "Bought" : "Sold"} ${quantity} ${asset.symbol} @ ${fmtMoney(asset.current_price)}`);
    onOpenChange(false);
    setQty("1");
    onDone?.();
  };
        ...
        <div className="space-y-4">
          ...
          <Button
            ...
            className={`w-full font-mono uppercase tracking-wider ${
              isBuy ? "bg-gain text-gain-foreground hover:bg-gain/90" : "bg-loss text-loss-foreground hover:bg-loss/90"
            }`}
          >
